...
public class Pagination {
	...
	private int currentPageBlock = 1; // 현재 페이지 블록
	
	public int getStartPageNoInBlock() {
		return (getCurrentPageBlock() - 1) * pageCntPerBlock + 1; // 현재 페이지 블록의 시작 페이지 번호
	}
	
	public int getEndPageNoInBlock() {
		return getStartPageNoInBlock() + pageCntPerBlock - 1;// // 현재 페이지 블록의 마지막 페이지 번호
	}
	
	public int getStartIndex() { 
		return (currentPageNo - 1) * itemsCntPerPage; // 해당 페이지의 게시물 목록의 첫 인덱스
	}
	public int getEndIndex() { 
		return getStartIndex() + itemsCntPerPage; // 해당 페이지의 게시물 목록의 마지막 인덱스
	}
